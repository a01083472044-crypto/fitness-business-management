...
function getQ(m: string) { return Math.ceil(Number(m.split("-")[1]) / 3); }
function getYear(m: string) { return Number(m.split("-")[0]); }
...
export default function TaxPage() {
  ...
  const [selectedQ, setSelectedQ] = useState(getCurrentQ());
  ...
  const qMonths = useMemo(() => getQMonths(selectedQ, selectedYear), [selectedQ, selectedYear]);
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 p-5 space-y-4">
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4].map((q) => (
                    <button key={q} onClick={() => setSelectedQ(q)}
                      className={`w-10 py-1.5 rounded-xl text-xs font-bold border transition ${
          ...
          {bizType === "간이" ? (
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl bg-zinc-50 p-3 text-xs text-zinc-500 space-y-1">
                <p className="font-semibold">부가세 신고 주요 항목</p>
                <p>· 매출세액: 공급가액의 10%</p>
                <p>· 매입세액 공제: 세금계산서 수취분 차감</p>
                <p>· 납부세액 = 매출세액 − 매입세액</p>
